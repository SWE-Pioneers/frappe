...
- Produces:
  ...
  - `set_default_accounts(company: str) -> None` — from the `coa_defaults.py` `{account_number: company_field}` map, wire Company defaults + Mode-of-Payment accounts: 1131→`default_receivable_account`, 2111→`default_payable_account`, 5111→`default_expense_account` (COGS), 4110→`default_income_account`, 1141→`default_inventory_account`, 2141→`stock_received_but_not_billed`, 1146→`default_provisional_account`/SDBNB, 5123→`stock_adjustment_account`, 5121→`expenses_included_in_valuation`, 5291→`round_off_account`, 5292→round-off-for-opening, 1910→temporary-opening, 3310→retained earnings, 5282→`write_off_account`, 5263→`exchange_gain_loss_account`, 5251→depreciation, 5254→`disposal_account`; Mode of Payment Cash→1111, Bank Transfer→1121. (Exact Company fieldnames verified in Task 1 `INTEGRATION_NOTES.md`.)
